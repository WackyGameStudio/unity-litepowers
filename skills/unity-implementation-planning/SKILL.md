...
8. Read `references/architecture-pattern-decision-matrix.md` for architecture-heavy tasks.
9. Read `references/dots-ecs-decision-matrix.md` when large entity counts, CPU hot paths, deterministic simulation, streaming, heavy AI/resource simulation, or large multiplayer are plausible.
10. Read `references/mcpforunity-evidence-matrix.md` for Unity surface evidence.
11. Ask only missing high-cost decisions.
12. For each architecture, SOLID, pattern, DOTS/ECS, Unity surface, or verification decision, present exactly two context-suitable options, recommendation, reason, and evidence note.
13. Use source/project/web search before asking when option quality depends on facts not in docs.
14. Do not invent filler choices; if only one option is viable, ask for confirmation.
15. Write technical plan, milestones, and task tickets with surfaces, Architecture / Pattern Fit, DOTS / ECS Suitability when relevant, verification profile, and docs update effects.
16. For feature work, save `iterations/YYYY-MM-DD-HHmmss-implementation-plan.md` before updating canonical `implementation-plan.md`.
17. Call `risk-based-verification-unity` for each task.
18. End with a Handoff block that names `executing-unity-plan-lite` when ready.
