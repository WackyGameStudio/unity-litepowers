...
4. Read relevant templates in `references/`.
5. Fill every required system field.
6. Skip questions already answered by docs.
7. Ask only missing player-facing, system, content, UI, or Unity design decisions.
8. For each decision question, present exactly two context-suitable options, recommendation, reason, and evidence note.
...
11. For feature work, save `iterations/YYYY-MM-DD-HHmmss-game-design.md` before updating canonical `game-design.md`.
12. Save to main `game-design.md` only for first creation or approved main update.
13. End with a Handoff block that names `unity-implementation-planning` when ready.
